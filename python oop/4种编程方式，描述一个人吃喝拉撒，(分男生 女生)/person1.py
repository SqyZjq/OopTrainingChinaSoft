...
"""
实现一个人的   长身体 小便 的功能。
n多个辅助函数，然后暴露一个公有函数供外部调用，这样无需在调用处写好几行调用各种证件辅助方法和保存各种证件变量。但是当某个辅助函数需要改变时候，除了全盘复制粘贴扣字，还有什么办法？
...
if __name__ == '__main__':
    ...
    具有调用方式上有一致性，也实现了男女的小便不同之处，但为了一个小的不同，而去复制粘贴全流程，然后扣字，这样简直不要太low，每当需要修改某个地方时候，这两个文件都需要去修改一下。复制粘贴扣字两三次还好，要是复制粘贴扣字几百次，那就很伤心了。
