...
和person_f文件的写法是一样的。
下面这3个类分别为实例方法类 类方法类 静态方法类，但写成了需要反复重复传参和大量return，这3个全部都是废物类，不是真oop
实例方法: 第一个参数self-代表类的实例，实例方法的调用-通过实例对象
类方法: 第一个参数cls-代表类，类方法的调用-通过类名.方法名()或实例来用
静态方法：使用@staticmethod装饰器，无默认参数，静态方法调用-通过类名.方法名()或实例来用
"""
